...
from ml_app.forms import VideoUploadForm
from .models import Item
# from forms import VideoUploadForm


index_template_name = 'index.html'
# predict_template_name ='/home/kmit-nvidia/deep_fake/Deepfake_detection_using_deep_learning/Django_app/ml_app/templates/predict.html'
...
def predict_page(request):
    ...
    if request.method == "GET":
        ...
        # clearing contents of previous files from static -> frames,org_videos,cropped_faces

        # print(str(settings.PROJECT_DIR+'/static/org_video/'))

        # ----------------- remove on filled--------------------
        
        for file in os.listdir(settings.PROJECT_DIR+'/static/org_video/'):
            os.remove(settings.PROJECT_DIR+'/static/org_video/'+file)
        
        for folder in os.listdir(settings.PROJECT_DIR+'/static/frames/'):
            for file in os.listdir(os.path.join(settings.PROJECT_DIR+'/static/frames/',folder)):
                os.remove(os.path.join(os.path.join(settings.PROJECT_DIR+'/static/frames',folder),file))
            os.rmdir(os.path.join(settings.PROJECT_DIR+'/static/frames/',folder))
        
        for folder in os.listdir(settings.PROJECT_DIR+'/static/cropped_faces/'):
            for file in os.listdir(os.path.join(settings.PROJECT_DIR+'/static/cropped_faces/',folder)):
                os.remove(os.path.join(os.path.join(settings.PROJECT_DIR+'/static/cropped_faces',folder),file))
            os.rmdir(os.path.join(settings.PROJECT_DIR+'/static/cropped_faces/',folder))
        ...
        #copy video file 
        # /home/kmit-nvidia/deep_fake/Deepfake_detection_using_deep_learning/Django_app/uploaded_videos/uploaded_file_1661193531.mp4
        temp=str(video_file+'.')[:-1]
        video_file_name=temp.split('/')[-1]
        shutil.copy(video_file,settings.PROJECT_DIR+'/static/org_video/'+video_file_name )
        ...
        video_file_name_only=video_file_name.split('.')[0]
        # print(os.path.join(os.path.join(os.path.join(os.path.join(settings.PROJECT_DIR,'static')),'frames'),video_file_name_only))
        video_file_location =os.path.join(os.path.join(os.path.join(settings.PROJECT_DIR,'static'),'org_video'),video_file_name)
        video_frames_folder=os.path.join(os.path.join(os.path.join(os.path.join(settings.PROJECT_DIR,'static')),'frames'),video_file_name_only)
        video_cropped_images_folder=os.path.join(os.path.join(os.path.join(os.path.join(settings.PROJECT_DIR,'static')),'cropped_faces'),video_file_name_only)
        
        new_db_entry = Item(video_file_name=video_file_location,video_frames_name=video_frames_folder,video_cropped_name=video_cropped_images_folder)

        new_db_entry.save()

        os.mkdir(os.path.join(os.path.join(os.path.join(os.path.join(settings.PROJECT_DIR,'static')),'frames'),video_file_name_only))
        os.mkdir(os.path.join(os.path.join(os.path.join(os.path.join(settings.PROJECT_DIR,'static')),'cropped_faces'),video_file_name_only))

        for i in range(1, sequence_length+1):
            frame = frames[i]
            image = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            img = pImage.fromarray(image, 'RGB')
            # image_name = "Django_app/ml_app/uploaded_videos/"+str(video_file_name_only).split("/")[7]+"_preprocessed_"+str(i)+'.png'
            ...
            #     # image_path = os.path.join(image_nam)

            # else:
            # print("image_name",image_name)
            image_path =  os.path.join(os.path.join(os.path.join(os.path.join(os.path.join(settings.PROJECT_DIR,'static')),'frames'),video_file_name_only),image_name)
            # print("path_saving",image_path)
            # print(image_path)
            img.save(image_path)
            preprocessed_images.append( os.path.join(os.path.join('frames/',video_file_name_only),image_name))
            # print(os.path.join(os.path.join('frames/',video_file_name_only),image_name))
        ...
        for i in range(1, sequence_length+1):
            ...
            # image_path = settings.PROJECT_DIR+"/static/cropped_faces/" + image_name
            image_path =  os.path.join(os.path.join(os.path.join(os.path.join(os.path.join(settings.PROJECT_DIR,'static')),'cropped_faces'),video_file_name_only),image_name)

            img.save(image_path)
            faces_found = faces_found + 1
            faces_cropped_images.append(os.path.join(os.path.join('cropped_faces',video_file_name_only),image_name))
    ...
    # tr
        ...
        return render(request, predict_template_name, {'preprocessed_images': preprocessed_images, 'heatmap_images': heatmap_images, "faces_cropped_images": faces_cropped_images, "original_video": 'org_video/'+video_file_name, "models_location": models_location, "output": output, "confidence": confidence})
        # else:
